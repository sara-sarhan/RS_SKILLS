...
class skills_extraction():
    def __init__(self, pdfnamepath, pathmodel, folder):
        ...
        full_occupations_skills['essential_skills'] = full_occupations_skills['essential_skills'].apply(
            lambda x: ','.join(str(x).split('; ')))
        full_occupations_skills['optional_skills'] = full_occupations_skills['optional_skills'].apply(
            lambda x: ','.join(str(x).split('; ')))

        full_occupations_skills_clean = pd.DataFrame(
            {'occupations_skillset': full_occupations_skills['essential_skills']})
        l = []
        ...
        self.namesprogramming.extend(
            ["Eclipse", "Java" "J2EE", "HTML", "JSP", " JAX RPC", "JAXB", "CSS3", "JavaScript", "jQuery", "Spring MVC",
             "Hibernate", " RESTful web services", "Apache Tomcat", "Cucumber", "Cassandra", "Junit", "Jenkins",
             "Maven", "GitHub", "XML", "Log4j", "EJB", "MySQL", "Ajax"])
    ...
    def extract_company(self, text):
        text = text.replace(':', ' ').lower().strip().replace(',', ' ')
    ...
    def extract_date(self, text, last):
        ...
        data = re.findall(
            "(((\d{1,4}([.\-/])([.\-/])\d{1,4}\s)(nascita|nata|data di nascita|nato|nascita|di nascita))|(\d{1,4}\s(gennaio|febbraio|marzo|aprile|maggio|giugno|luglio|agosto|settembre|ottobre|novembre|dicembre)\s\d{1,4})|(nascita|nata|data di nascita|nato|di nascita) ((\d{1,4}([.\-/])\d{1,2}([.\-/])\d{1,4}))|(nascita|nata|data di nascita|nato|di nascita) ((\s\d{1,4}([.\-/])\d{1,2}([.\-/])\d{1,4})))",
            text)
    ...
    def ita_estrcation_jobs(self, sentece):
        ...
        if ('assegnista' not in sentece and 'lavoro' not in sentece and 'ricercatore' not in sentece):
            for study in ['diploma', 'corso formazione', 'liceo', 'corso aggiornamneto', 'attestato conseguito',
                          'stage', 'borsista', 'laurea', 'certificazione', 'tirocinio', 'facoltà', 'facolta',
                          'scuola', 'università', 'liceo', 'diploma', 'dottorato', 'master',
                          'istituto technico']:  # corso
                span_list = [(match.start(), match.end()) for match in
        ...
        for testskill in resume_sections:
            ...
            if span_list:
                if len(testskill) > 1:
                    ...
                    skillmach.append(skillfound)
                    if skillfound.strip() != '' and skillfound not in self.stop and skillfound not in skillmach and len(
                            skillfound.split()) > 1 and skillfound not in worddelete:
            ...
            if doc:
                ...
                if len(verb) == 1 and " ".join(sentece).strip() != '' and len(sentece) > 1 and " ".join(
                        sentece).strip() != 'capacità sono maturate':
    ...
    def ita_skills_formsentece(self, frasi):
        ...
        listacompetenze = ['skills', 'addetto al', 'addetta al', 'specializzato in', 'specializzata in', "conoscenza",
                           "competenze", "capacità", 'predisposizione al', "specializzato nell", "specializzata nell"]
        worddelete = ['capacità e competenze personali', 'patente guida',
                      "capacità di lettura  capacità di scrittura  capacità di espressione orale",
                      "capacità di lettura", "capacità di scrittura", "capacità di espressione orale", 'capacità di',
                      'competenze personali', 'competenze informatiche', 'capacità e competenze informatiche',
    ...
    def esxtractin_progra_skills(self, frase):
        ...
        for s in self.namesprogramming:
            s = s.replace('+', ' plus').replace('++', 'plus').replace('(', ' ').replace(')', ' ').replace('* ',
                                                                                                          ' ').replace(
                '+2', ' ').strip()
        ...
        print("end extract programming skills")
    ...
    def pdf_to_html(self):
        ...
        it_job_posts['jobDescription'] = it_job_posts['jobDescription'].apply(
            lambda x: re.sub(r'[^\w\s]', '', x).strip().lower().replace(',', ''))
        ...
        exp = [experience for experience in job if
               " ".join(experience) not in ['corso formazione', 'corso', 'attestato conseguito', 'stage', 'laurea',
                                            'facoltà', 'facolta', 'scuola', 'università', 'liceo', 'diploma',
                                            'dottorato',
        ...
        for experience, text in zip(job, test):
            ...
            if len(experience) > 0 and ",".join(experience) not in ['corso formazione', 'corso', 'attestato conseguito',
                                                                    ...
                                                                    'università', 'liceo', 'diploma', 'dottorato',
                                                                    'master',
                                                                    'istituto technico']:
